...
<meta name="keywords" content="{{ $getInfo->getValue('main_keywords') }}, {{ $nTags }}, {{ $nCategories }}">
<meta name="author" content="{{ $user->find($post->uid)->firstname }} {{ $user->find($post->uid)->lastname }}">
<meta name="author" content="{{ $post->author }}">
<meta property="og:title" content="{{ $post->title }}" />
<meta property="og:type" content="blog.news" />
<meta property="og:url" content="{{ Request::url() }}" />
<meta property="og:image" content="https://english.mawajez.com/{{ $post->image }}" />
...
@if (strpos($post->image, 'http') !== true)
    <header class="masthead ddkits-trans container" style="margin-top:75px;background: url('/{{ (!empty($post->image) ? $post->image : env('APP_URL').'/img/notFoundImage.jpg') }}') no-repeat;
      ...
      <div class="container ">
        ...
        <div class="ddkits-blog-content col-md-12">
          <section>
                  ...
                  @include('includes.google-ad')
                  @if($posts->nCategories($post->id, 'blog'))
                  ...
                  @endif
                  <p>By: <a href="/profile/{{ $getProfile->getProfInfo($post->uid)->id }}">{{ $user->find($post->uid)->firstname }} {{ $user->find($post->uid)->lastname }}</a></p>
            </div>
        </div>
      </div>
    </div>
